...
async def handle_message(websocket, path,server,step_count):
    while True:
        ...
        elif json_message['type']=='add_next_documents_for_user_studies':
            step_count = step_count + 1
            if step_count == 1:
                data_zotero = server.Zotero("getCollectionItems",'5WYEJX3C',False,False)
            elif step_count ==2:
                data_zotero = server.Zotero("getCollectionItems",'QFD2CSZY',False,False)
            collection_items = json.loads(data_zotero)
            pdf_ids = []
            for  i in range(0,len(collection_items)):
                pdf_ids.append(collection_items[i]['pdf_file'])
