...
def topic_extractor(data_df,type_of_extraction):
    ...
    #we are working with abstracts now
    text_to_extract = 'abstract'
    ...
    # pdb.set_trace()
    data_df = data_df[data_df[text_to_extract] != 'Parsing Error']
    ...
    if type_of_extraction == 'session':
        with open(text_fn[lang], 'w', encoding='utf-8') as fp:
            for record_text in data_df[text_to_extract]:
